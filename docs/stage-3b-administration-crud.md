...
Todas las acciones viven en `modules/platform/admin/application/actions.ts`, validan `permissions.key` y usan un cliente `server-only`. Las creaciones fallidas compensan Auth, perfil y asignaciones. La desactivación del último Superadministrador y de roles con usuarios activos queda bloqueada.
