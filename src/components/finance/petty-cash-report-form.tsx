...
export function PettyCashReportForm({
  ...
  weeklySummaries?: Record<
    string,
    { weekly_limit?: number; committed?: number; available?: number } | null
  >;
}) {
  const router = useRouter();
  const [state, action, pending] = useActionState(savePettyCashDraftAction, {
    success: false,
    message: "",
  } as PettyCashActionResult);
  ...
  const initialLines = (
    (report?.petty_cash_expense_lines as
  ...
  const [unitId, setUnitId] = useState(
    String(report?.business_unit_id ?? units[0]?.id ?? ""),
  );
  const [selectedWeek, setSelectedWeek] = useState(() => ({
    start: String(report?.week_start ?? week.start),
    end: String(report?.week_end ?? week.end),
  }));
  const [lines, setLines] = useState<Line[]>(
    initialLines.length ? initialLines : [blankLine(selectedWeek.start, 0)],
  );
  ...
  const summaryKey = `${unitId}:${selectedWeek.start}`;
  const summaryLoading =
    selectedWeek.start !== week.start && loadedSummary?.key !== summaryKey;
  const selectedSummary =
    loadedSummary?.key === summaryKey
      ? loadedSummary.value
      : selectedWeek.start === week.start
        ? (weeklySummaries?.[unitId] ?? weeklySummary)
        : null;
  const available = Number(
    selectedSummary?.available ?? selectedSummary?.weekly_limit ?? 100000,
  );
  useEffect(() => {
    let active = true;
    void pettyCashWeekSummaryAction(unitId, selectedWeek.start).then(
      (value) => {
        if (active) setLoadedSummary({ key: summaryKey, value });
      },
    );
    return () => {
      active = false;
    };
  }, [unitId, selectedWeek.start, summaryKey]);
  useEffect(() => {
    if (!state.success || !state.id || handledState.current === state) return;
    handledState.current = state;
    const files = selectedFiles;
    const targets =
      (state.data as { uploadTargets?: UploadTarget[] } | undefined)
        ?.uploadTargets ?? [];
    const uploadReceipts = async () => {
      ...
      try {
        ...
        if (totalFiles)
          setUploadMessage(`Subiendo ${totalFiles} comprobante(s)…`);
        ...
        for (const target of targets) {
          for (const file of files[target.client_id] ?? []) {
            ...
            const preparedData = prepared.data as
              { attachment_id?: string; object_path?: string } | undefined;
            if (!prepared.id || !preparedData?.object_path)
              throw new Error("No fue posible preparar el comprobante.");
            ...
            if (error) {
              ...
              throw new Error(
                "No fue posible subir uno de los comprobantes. Intenta nuevamente.",
              );
            }
          }
        }
        ...
        setUploadMessage(
          totalFiles
            ? "Borrador y comprobantes guardados correctamente."
            : state.message,
        );
        ...
        setUploadError(
          error instanceof Error
            ? error.message
            : "No fue posible guardar los comprobantes.",
        );
      } finally {
        setUploading(false);
      }
  ...
  const remove = (index: number) => {
    ...
    if (confirm("¿Eliminar esta línea de gasto?"))
  ...
  return (
    ...
    >
      ...
      <input type="hidden" name="week_start" value={selectedWeek.start} />
      <input type="hidden" name="week_end" value={selectedWeek.end} />
      ...
      <section className="rounded-2xl border bg-white p-5">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <Field label="Unidad de negocio">
            ...
            {Boolean(report?.id) && (
              <input type="hidden" name="business_unit_id" value={unitId} />
            )}
          </Field>
          <Field label="Semana" error={state.fieldErrors?.week_start?.[0]}>
            {report?.id ? (
              <input
                readOnly
                value={formatWeek(selectedWeek.start, selectedWeek.end)}
              ...
              <>
                  ...
                  onChange={(event) => {
                    ...
                    setLines((current) =>
                      current.map((line) => ({
                        ...line,
                        expense_date:
                          line.expense_date >= next.start &&
                          line.expense_date <= next.end
                            ? line.expense_date
                            : next.start,
                      })),
                    );
                ...
                <span className="mt-1 block text-xs text-slate-500">
                  {formatWeek(selectedWeek.start, selectedWeek.end)} · Puedes
                  elegir una semana anterior para regularizarla.
                </span>
              </>
            )}
          </Field>
          <div className="md:col-span-2">
            <Field
              ...
              error={state.fieldErrors?.general_reason?.[0]}
            >
              <input
                name="general_reason"
                defaultValue={String(report?.general_reason ?? "")}
                minLength={3}
                maxLength={500}
                required
                className={input}
                placeholder="Ej.: Gastos operacionales de la semana"
              />
            </Field>
          </div>
          <div className="md:col-span-2">
            <Field label="Observaciones generales (opcional)">
              <textarea
                ...
                defaultValue={String(report?.general_observations ?? "")}
                maxLength={1000}
                rows={3}
                className={input}
              />
      ...
      <div className="sticky top-3 z-10 grid grid-cols-3 gap-2 rounded-2xl border bg-white/95 p-3 text-center text-xs shadow-lg backdrop-blur md:ml-auto md:max-w-xl">
        <Metric
          label="Total borrador"
          value={clp(total)}
          danger={!summaryLoading && total > available}
        />
        <Metric
          label="Saldo disponible"
          value={summaryLoading ? "Calculando…" : clp(available)}
        />
        <Metric
          label="Saldo proyectado"
          value={
            summaryLoading ? "Calculando…" : clp(Math.max(0, available - total))
          }
          danger={!summaryLoading && total > available}
        />
      </div>
      ...
      <div className="space-y-4">
        {lines.map((line, index) => (
          <section
            key={line.client_id}
            className={`rounded-2xl border bg-white p-4 md:p-5 ${line.review_status === "observed" ? "border-orange-400" : ""}`}
          >
            ...
            {line.reviewer_comment && (
              <p className="mt-3 rounded-xl bg-orange-50 p-3 text-sm text-orange-900">
                <b>Observación del revisor:</b> {line.reviewer_comment}
              </p>
            )}
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              <Field label="Fecha del gasto">
                <input
                  ...
                  min={selectedWeek.start}
                  max={selectedWeek.end}
                  onChange={(e) =>
                    update(index, "expense_date", e.target.value)
                  }
                  required
                  className={input}
                />
              </Field>
              <Field label="Comercio o proveedor">
                <input
                  value={line.merchant_name}
                  onChange={(e) =>
                    update(index, "merchant_name", e.target.value)
                  }
                  minLength={2}
                  required
                  className={input}
              ...
              <Field label="Tipo de documento">
                <select
                  ...
                  onChange={(e) =>
                    update(index, "document_type", e.target.value)
                  }
                  className={input}
                >
              ...
              <Field label="Número de documento (opcional)">
                <input
                  value={line.document_number}
                  onChange={(e) =>
                    update(index, "document_number", e.target.value)
                  }
                  className={input}
                />
              </Field>
              <Field label="Categoría">
                <select
                  ...
                  onChange={(e) =>
                    update(index, "expense_category_id", e.target.value)
                  }
                  required
                  className={input}
                >
              ...
              <Field label="Centro de costo">
                <select
                  ...
                  onChange={(e) =>
                    update(index, "cost_center_id", e.target.value)
                  }
                  required
                  className={input}
                >
              ...
              <div className="md:col-span-2">
                <Field label="Observación (opcional)">
                  <input
                    ...
                    onChange={(e) =>
                      update(index, "observation", e.target.value)
                    }
                    maxLength={500}
                    className={input}
                  />
              ...
              <Field
                label={`Comprobante ${line.attachments?.length ? "adicional" : "obligatorio al enviar"}`}
              >
                <input
            ...
            {(Boolean(line.attachments?.length) ||
              Boolean(selectedFiles[line.client_id]?.length)) && (
              <div className="mt-4 rounded-xl bg-slate-50 p-3">
                ...
                <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  {(selectedFiles[line.client_id] ?? []).map((file) => (
                    <ReceiptPreview
