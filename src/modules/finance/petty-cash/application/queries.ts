...
export async function currentWeekSummary(
  unitId?: string,
  responsibleId?: string,
  targetWeek = chileWeek().start,
) {
...
export async function activeCatalogs() {
  ...
  const [{ data: categories }, { data: centers }] = await Promise.all([
    supabase
      ...
      .select("id,name,company_id,business_unit_id")
...
export async function signPettyCashAttachments(
  ...
  return Promise.all(
    lines.map(async (line): Promise<Record<string, unknown>> => ({
      ...line,
      petty_cash_line_attachments: await Promise.all(
        (
          (line.petty_cash_line_attachments as
            Array<Record<string, unknown>> | undefined) ?? []
        )
          ...
          .map(async (attachment): Promise<Record<string, unknown>> => {
            const { data } = await supabase.storage
              ...
              .createSignedUrl(String(attachment.object_path), 300, {
                download: String(attachment.original_name),
              });
