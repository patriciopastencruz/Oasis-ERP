...
export async function pettyCashReport(params: URLSearchParams) {
  ...
  if (
    !ctx.permissions.has("finance.petty_cash.reports.view") &&
  ...
  let query = supabase
    ...
    .is("deleted_at", null);
  if (params.get("unit"))
    query = query.eq("business_unit_id", params.get("unit")!);
  if (params.get("worker"))
    query = query.eq(
      "petty_cash_reports.responsible_id",
      params.get("worker")!,
    );
  if (params.get("week"))
    query = query.eq("petty_cash_reports.week_start", params.get("week")!);
  if (params.get("status"))
    query = query.eq("petty_cash_reports.status", params.get("status")!);
  if (params.get("category"))
    query = query.eq("expense_category_id", params.get("category")!);
  if (params.get("center"))
    query = query.eq("cost_center_id", params.get("center")!);
  if (params.get("merchant"))
    query = query.ilike("merchant_name", `%${params.get("merchant")}%`);
  if (params.get("document_type"))
    query = query.eq("document_type", params.get("document_type")!);
  const { data, error } = await query
    .order("expense_date", { ascending: false })
    .limit(10000);
  if (error)
    throw new Error("No fue posible generar el reporte de Caja Chica.");
  return (data ?? []).map((line): PettyCashExportRow => {
    ...
    const approver = one<{ first_name?: string; last_name?: string }>(
      report.approver,
    );
    ...
    return {
      correlativo: String(report.report_number ?? "Borrador"),
      trabajador:
        `${worker?.first_name ?? ""} ${worker?.last_name ?? ""}`.trim(),
      unidad: unit?.name ?? "",
      semana: `${String(report.week_start)} / ${String(report.week_end)}`,
      fecha_gasto: line.expense_date,
      comercio: line.merchant_name,
      tipo_documento: line.document_type,
      numero_documento: line.document_number ?? "",
      categoria: category?.name ?? "",
      centro_costo: center?.name ?? "",
      descripcion: line.description,
      monto: Number(line.amount),
      estado: uiLabel(report.status),
      administrador:
        `${approver?.first_name ?? ""} ${approver?.last_name ?? ""}`.trim(),
      fecha_aprobacion: report.approved_at ? String(report.approved_at) : "",
      observaciones: line.observation ?? "",
