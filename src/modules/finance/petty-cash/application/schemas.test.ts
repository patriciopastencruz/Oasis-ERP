...
describe("validación de rendiciones", () => {
  it("acepta una línea completa", () =>
    expect(expenseLineSchema.safeParse(line).success).toBe(true));
  it("rechaza montos cero", () =>
    expect(expenseLineSchema.safeParse({ ...line, amount: 0 }).success).toBe(
      false,
    ));
  ...
  it("rechaza gastos fuera de la semana informada", () => {
    const result = reportDraftSchema.safeParse({
      business_unit_id: "33333333-3333-4333-8333-333333333333",
      week_start: "2026-07-13",
      week_end: "2026-07-19",
      general_reason: "Regularización semanal",
      lines: [{ ...line, expense_date: "2026-07-12" }],
    });
    expect(result.success).toBe(false);
  });
  it("exige una semana de lunes a domingo", () => {
    const result = reportDraftSchema.safeParse({
      business_unit_id: "33333333-3333-4333-8333-333333333333",
      week_start: "2026-07-14",
      week_end: "2026-07-20",
      general_reason: "Regularización semanal",
      lines: [{ ...line, expense_date: "2026-07-14" }],
    });
    expect(result.success).toBe(false);
  });
