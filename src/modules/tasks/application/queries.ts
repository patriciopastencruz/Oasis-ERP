...
export async function loadBoard(companyId: string): Promise<BoardCard[]> {
  ...
  const { data, error } = await supabase
    .from("task_cards")
    .select(
      "id,title,description,status,due_date,sort_order,assignee:profiles!task_cards_assignee_id_fkey(id,first_name,last_name),business_unit:business_units!task_cards_business_unit_fkey(id,name)",
    )
